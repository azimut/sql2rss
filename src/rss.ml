...
let print_header () =
  tag "title" title ;
  tag "link" link ;
  print_endline @@ "<atom:link href=\"" ^ link
...
let print_entry (entry : Sql.t) =
  let anchorify (s : string) =
    let mapanchor w =
      if String.starts_with ~prefix:"http" w then
        Printf.sprintf "<a href='%s'>%s</a>" w w
      else w
    in
    s |> String.split_on_char ' ' |> List.map mapanchor |> String.concat " "
  in
  let print_author () =
    Printf.printf "<author><name>%s</name></author>\n" entry.window
  in
